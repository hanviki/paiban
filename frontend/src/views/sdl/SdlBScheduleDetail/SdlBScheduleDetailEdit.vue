...
export default {
  ...
  methods: {
    ...
    setFormValues({ ...sdlBScheduleDetail }) {
      let fields = [
        "baseId",
        "deptName",
        "yqName",
      ...
      let fieldDates = ["scheduleDate"];
      Object.keys(sdlBScheduleDetail).forEach((key) => {
        if (fields.indexOf(key) !== -1) {
          this.form.getFieldDecorator(key);
          let obj = {};
          if (fieldDates.indexOf(key) !== -1) {
            if (sdlBScheduleDetail[key] !== "") {
              obj[key] = moment(sdlBScheduleDetail[key]);
            } else {
              obj[key] = "";
            }
          } else {
            obj[key] = sdlBScheduleDetail[key];
          }
          this.form.setFieldsValue(obj);
        }
      });
      this.sdlBScheduleDetail.id = sdlBScheduleDetail.id;
    },
    handleSubmit() {
      this.form.validateFields((err, values) => {
        if (!err) {
          let sdlBScheduleDetail = this.form.getFieldsValue();
          sdlBScheduleDetail.id = this.sdlBScheduleDetail.id;
          this.$put("sdlBScheduleDetail", {
            ...sdlBScheduleDetail,
          })
            .then(() => {
              this.reset();
