...
export default {
  ...
  methods: {
    ...
    setFormValues({ ...sdlBScheduleD }) {
      ...
      Object.keys(sdlBScheduleD).forEach((key) => {
        if (fields.indexOf(key) !== -1) {
          this.form.getFieldDecorator(key);
          let obj = {};
          if (fieldDates.indexOf(key) !== -1) {
            if (sdlBScheduleD[key] !== "") {
              obj[key] = moment(sdlBScheduleD[key]);
            } else {
              obj[key] = "";
            }
          } else {
            obj[key] = sdlBScheduleD[key];
          }
          this.form.setFieldsValue(obj);
        }
      });
      this.sdlBScheduleD.id = sdlBScheduleD.id;
    },
    handleSubmit() {
      this.form.validateFields((err, values) => {
        if (!err) {
          let sdlBScheduleD = this.form.getFieldsValue();
          sdlBScheduleD.id = this.sdlBScheduleD.id;
          this.$put("sdlBScheduleD", {
            ...sdlBScheduleD,
          })
            .then(() => {
              this.reset();
