  ...
  >
    <a-form :form="form">
      ...
      <div style="margin: 5px 0px">
        <a-row type="flex" justify="center">
          <a-col :span="colSpan">
            <xxbBProj-dept ref="xxbBProjDeptList" :deptData="deptData"> </xxbBProj-dept>
          </a-col>
        </a-row>
      </div>
      <!-- 项目主要人员 -->
      ...
      <div style="margin: 5px 0px">
        <a-row type="flex" justify="center">
          <a-col :span="colSpan">
            <xxbBLcmany-d ref="xxbBLcmanyDList" @jisuan="jisuan"> </xxbBLcmany-d>
          </a-col>
        </a-row>
      </div>
      <!-- 项目主要人员情况 -->
...
import XxbBLcmanyD from "../XxbBLcmany/XxbBLcmanyD";
import XxbBProjDept from "../XxbBLcmany/XxbBProjDept";
...
export default {
  ...
  methods: {
    ...
    setFormValues(xxbBCheck,tit,projectType) {
      ...
      setTimeout(() => {
        this.$refs.xxbBLcmanyDList.setFieldValues(xxbBCheck == null? null : xxbBCheck.id,this.isEdit);
        this.$refs.xxbBProjDeptList.setFieldValues(xxbBCheck == null? null : xxbBCheck.id,this.isEdit);
        this.$refs.xxbBDeptFlowLook.setFieldValues(xxbBCheck == null? null : xxbBCheck.id,
        2,xxbBCheck == null? 0 : xxbBCheck.state,0);
      }, 100);
    ...
    handleSubmit(type) {
      ...
      this.form.validateFields((err, values) => {
        if (!err) {
          ...
          xxbBCheck.projectType = 2
          xxbBCheck.userdept = this.xxbBCheck.userdept;
          xxbBCheck.checkDataList = this.$refs.xxbBLcmanyDList.getFieldValues();
          xxbBCheck.projDeptList = this.$refs.xxbBProjDeptList.getFieldValues();
          if (type != 0) {
            let sumGxd = 0;
            xxbBCheck.projDeptList.forEach((item,index) => {
              sumGxd += item.gxd
            });
            if (sumGxd != 1) {
              this.$message.error(
            ...
            data: JSON.stringify(xxbBCheck),
              ...
              if (r.data.data.success === 1) {
                ...
                if (type == 1) {
                  ...
                  this.$refs.xxbBLcmanyDList.setFieldValues(
                    this.xxbBCheck.id,this.isEdit);
                  this.$refs.xxbBProjDeptList.setFieldValues(
                    this.xxbBCheck.id,this.isEdit);
