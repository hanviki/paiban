  ...
  <a-card :bordered="false" class="card-area">
        ...
        <a-row>
         ...
         show-search>
            ...
            <template v-if="advanced">
              <a-col :md="8" :sm="24">
                <a-form-item label="操作等级" v-bind="formItemLayout">
    ...
    <div>
      <div class="operator">
        ...
        <import-excel
          templateUrl="mdlDOperation/downTemplate"
          @succ="handleRefesh"
          url="mdlDOperation/import"
        >
        </import-excel>
      </div>
      <!-- 表格区域 -->
...
<script>
import MdlDOperationAdd from "./MdlDOperationAdd";
import MdlDOperationEdit from "./MdlDOperationEdit";
...
export default {
  name: "MdlDOperation",
  components: { MdlDOperationAdd, MdlDOperationEdit, ImportExcel },
  data() {
    return {
      advanced: false,
      dataSource: [],
  ...
  methods: {
    ...
    edit(record) {
      this.$refs.mdlDOperationEdit.setFormValues(record);
    ...
    batchDelete() {
      ...
      this.$confirm({
        ...
        centered: true,
        onOk() {
          let mdlDOperationIds = that.selectedRowKeys.join(",");
          that.$delete("mdlDOperation/" + mdlDOperationIds).then(() => {
    ...
    exportExcel() {
      ...
      this.$export("mdlDOperation/excel", {
        sortField: sortField,
        sortOrder: sortOrder,
        ...this.queryParams,
      });
