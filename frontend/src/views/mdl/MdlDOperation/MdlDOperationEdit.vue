<template>
  ...
  >
     <a-form :form="form">
      ...
      <a-form-item v-bind="formItemLayout" label="操作名称">
        <a-input
          placeholder="请输入操作名称"
          v-decorator="[
            'name',
            { rules: [{ required: true, message: '操作名称不能为空' }] },
          ]"
        />
      </a-form-item>
      <a-form-item v-bind="formItemLayout" label="操作等级">
           <a-select  v-decorator="[
            'level',
            { rules: [{ required: true, message: '操作等级不能为空' }] },
          ]"
                 >
                   <a-select-option
                    value="一级"
...
export default {
  ...
  methods: {
    ...
    setFormValues({ ...mdlDSurgery }) {
      let fields = ["deptNew", "code", "name", "level", "note","lb","startDate"];
