...
export default {
  ...
  methods: {
    ...
    setFormValues({ ...mdlBArchive }) {
      ...
      Object.keys(mdlBArchive).forEach((key) => {
        if (fields.indexOf(key) !== -1) {
          this.form.getFieldDecorator(key);
          let obj = {};
          if (fieldDates.indexOf(key) !== -1) {
            if (mdlBArchive[key] !== "") {
              obj[key] = moment(mdlBArchive[key]);
            } else {
              obj[key] = "";
            }
          } else {
            obj[key] = mdlBArchive[key];
          }
          this.form.setFieldsValue(obj);
        }
      });
        this.mdlBArchive.fileId = mdlBArchive.fileId;
      this.mdlBArchive.fileUrl = mdlBArchive.fileUrl;
      this.mdlBArchive.id = mdlBArchive.id;
      let that=this
      setTimeout(()=>{
        that.$refs.fileagent.setForm(mdlBArchive.fileId);
      },100)
    },
    handleSubmit() {
