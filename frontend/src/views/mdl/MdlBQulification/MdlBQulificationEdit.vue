...
export default {
  ...
  methods: {
    ...
    setFormValues({ ...mdlBQulification }) {
      let fields = [
        "qlName",
        "fzDate",
        "bgDate",
      ...
      let fieldDates = ["fzDate", "bgDate"];
      Object.keys(mdlBQulification).forEach((key) => {
        if (fields.indexOf(key) !== -1) {
          this.form.getFieldDecorator(key);
          let obj = {};
          if (fieldDates.indexOf(key) !== -1) {
            if (mdlBQulification[key] !== "") {
              obj[key] = moment(mdlBQulification[key]);
            } else {
              obj[key] = "";
            }
          } else {
            obj[key] = mdlBQulification[key];
          }
          this.form.setFieldsValue(obj);
        }
      });
      this.mdlBQulification.id = mdlBQulification.id;
      this.mdlBQulification.fileId = mdlBQulification.fileId;
      this.mdlBQulification.fileUrl = mdlBQulification.fileUrl;
      var that =this
      setTimeout(()=>{
        that.$refs.fileagent.setForm(mdlBQulification.fileId);
      },100)
    ...
    handleSubmit() {
      this.form.validateFields((err, values) => {
        if (!err) {
          let mdlBQulification = this.form.getFieldsValue();
          mdlBQulification.id = this.mdlBQulification.id;
          mdlBQulification.fileId = this.mdlBQulification.fileId;
      mdlBQulification.fileUrl = this.mdlBQulification.fileUrl;
          this.$put("mdlBQulification", {
            ...mdlBQulification,
          })
            .then(() => {
              this.reset();
