<template>
  <div>
    <a-row>
      ...
      <a-col :span="5">
        <a-button class="editable-add-btn" v-if="isEdit" @click="handleAdd"> + 新增外派人员 </a-button>
      </a-col>
    </a-row>
    <a-table
      bordered
...
export default {
  ...
  methods: {
    ...
    fetch() {
      ...
      this.$get("mdlBForeign/findCheckD", {
        baseId: this.baseId,
      }).then((r) => {
      ...
      // if (newData.length > 1) {
        ...
        // if (target) {
          if (obj.id == null) {
            ...
            this.$get("mdlBForeign/delCheckD", {
              id: obj.id,
            }).then((r) => {
              if (r.data.data.success === 1) {
                this.$message.success("删除外派人员成功.");
                this.dataSource = [];
                this.fetch();
              } else {
                this.$message.error("删除外派人员失败.");
              }
            });
          }
        // }
...
record.displayIndex= value
    ...
    handleAdd() {
      ...
      //         } else {
      //           this.$message.error("增加外派人员失败.");
      //           this.cleanData();
      //         }
      //       } else {
      //         this.$message.error("增加外派人员失败或异常.");
      //         this.cleanData();
      //       }
      //     });
      //   }
